...
int main() {
	...
	printf("Dieser Programmteil prueft die Zahl, ob die Zahl gerade oder ungerade und ob sie eine Primzahl ist, und nimmt eine Primfaktorzerlegung vor:\n");
	// Abfragen der Zahl vom Benutzer und Ausgabe der eingegebenen Zahl 
	printf("Gib eine ganzzahlige positive Zahl (x>0) ein:\n");
	//int rueckgabewert = scanf("%u", &eingegebeneNummer);

	while (scanf("%u", &eingegebeneNummer) != 1){   //Überprüfung, ob eine entsprechende Zahl eingegeben wurde
		while (getchar() != '\n') {}
		printf("Fehler: Die Eingabe muss eine Zahl (x>0) sein...\n");
	} 
	...
	if (eingegebeneNummer % 2 == 0) {
		...
		if (eingegebeneNummer <= 1) {
			...
			// Primzahlprüfung: Zahl wird durch alle ungeraden Zahlen bis zur Zahl/2 geteilt, und wenn es eine Zahl mit %0 gibt das Programm abgebrochen
			for (unsigned int i = 3; i <= eingegebeneNummer / 2; i+= 2) {
	...
	// Erstellung der Primzahlliste, Bestimmung der einzelnen Primzahl und Lager in dem Array
		...
		for (unsigned int i = 2; i < eingegebeneNummer; i++) {
	...
	printf("Gib dafuer nun die erste ganze positive Zahl an:\n");

	while (scanf("%u", &eingegebeneZahl1) != 1) {   //Überprüfung, ob eine entsprechende Zahl eingegeben wurde
		while (getchar() != '\n') {}
		printf("Fehler: Die Eingabe muss eine Zahl (x>0) sein...\n");
	}
	printf("1.Zahl: %i\n", eingegebeneZahl1);

	printf("Gib dafuer nun die zweite ganze positive Zahl an:\n");
	while (scanf("%u", &eingegebeneZahl2) != 1) {   //Überprüfung, ob eine entsprechende Zahl eingegeben wurde
		while (getchar() != '\n') {}
		printf("Fehler: Die Eingabe muss eine Zahl (x>0) sein...\n");
	}
	printf("2.Zahl: %i\n", eingegebeneZahl2);
